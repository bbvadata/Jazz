...
put_R_block <- function(source, block_key, sexp, host = .host.)
{
	auxkey <- new_key()

	put_raw_block(source, auxkey, serialize(sexp, NULL), host)
	set_compatible_data_type(source, auxkey, type_const[['BLOCKTYPE_RAW_R_RAW']], host)

	downlo <- basicTextGatherer()
	if (curlPerform(url			  = paste0(host, '//', source, '.', block_key, '.from_R/', auxkey),
					infilesize	  = 0,
